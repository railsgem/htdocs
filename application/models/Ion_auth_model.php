...
class Ion_auth_model extends CI_Model
{
	...
	/**
	 * caching of users and their privileges
	 *
	 * @var array
	 **/
	public $_cache_user_in_privilege = array();

	/**
	 * caching of privileges
	 *
	 * @var array
	 **/
	protected $_cache_privileges = array();
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		{
			$user = $query->row();
			$myquery = "select mu.id,mu.li_id,mu.a_href,mu.i_class,mu.li_text,mu.display from 
						(select up.id,up.privilege_id,pm.menu_id from users_privileges up left join privileges_menu pm on up.privilege_id = pm.privilege_id
						where up.user_id='".$user->id."') pmu left join menu mu on pmu.menu_id = mu.id
						where pmu.privilege_id<>9
						order by mu.id";
	 ...
	 * @author Juno Chen
	 **/
	public function get_users_privileges($id=FALSE)
	{
		$this->trigger_events('get_users_privilege');

		//if no id was passed use the current users id
		$id || $id = $this->session->userdata('user_id');

		return $this->db->select($this->tables['users_privileges'].'.'.$this->join['privileges'].' as id, '.$this->tables['privileges'].'.name, '.$this->tables['privileges'].'.description')
		                ->where($this->tables['users_privileges'].'.'.$this->join['users'], $id)
		                ->join($this->tables['privileges'], $this->tables['users_privileges'].'.'.$this->join['privileges'].'='.$this->tables['privileges'].'.id')
		                ->get($this->tables['users_privileges']);
	}
	 ...
	 * @author Juno Chen
	 **/
	public function add_to_privilege($privilege_ids, $user_id=false)
	{
		$this->trigger_events('add_to_privilege');
		...
		{
			if ($this->db->insert($this->tables['users_privileges'], array( $this->join['privileges'] => (int)$privilege_id, $this->join['users'] => (int)$user_id)))
			{
				if (isset($this->_cache_privileges[$privilege_id])) {
					$privilege_name = $this->_cache_privileges[$privilege_id];
				}
				else {
					$privilege = $this->privilege($privilege_id)->result();
					$privilege_name = $privilege[0]->name;
					$this->_cache_privileges[$privilege_id] = $privilege_name;
				}
				$this->_cache_user_in_privilege[$user_id][$privilege_id] = $privilege_name;

				// Return the number of privileges added
				$return += 1;
			}
		}
	...
	/**
	 * remove_from_privilege
	 *
	 * @return bool
	 * @author Juno Chen
	 **/
	public function remove_from_privilege($privilege_ids=false, $user_id=false)
	{
		$this->trigger_events('remove_from_privilege');
		...
		// if privilege id(s) are passed remove user from the privilege(s)
		if( ! empty($privilege_ids))
		{
			...
			foreach($privilege_ids as $privilege_id)
			{
				$this->db->delete($this->tables['users_privileges'], array($this->join['privileges'] => (int)$privilege_id, $this->join['users'] => (int)$user_id));
				if (isset($this->_cache_user_in_privilege[$user_id]) && isset($this->_cache_user_in_privilege[$user_id][$privilege_id]))
				{
					unset($this->_cache_user_in_privilege[$user_id][$privilege_id]);
				}
			}

			$return = TRUE;
		}
		// otherwise remove user from all privileges
		else
		{
			if ($return = $this->db->delete($this->tables['users_privileges'], array($this->join['users'] => (int)$user_id))) {
				$this->_cache_user_in_privilege[$user_id] = array();
			}
		}
		return $return;
	}
	...
	{
		$this->trigger_events('privileges');
		//run each where that was passed
		if (isset($this->_ion_where) && !empty($this->_ion_where))
		{
			foreach ($this->_ion_where as $where)
		...
		$this->response = $this->db->get($this->tables['privileges']);
	...
	{
		$this->trigger_events('privilege');

		if (isset($id))
		{
			$this->where($this->tables['privileges'].'.id', $id);
		}
	...
	**/
	 ...
	 * @author Juno Chen
	 **/
	public function set_menu_session($menu)
	{
