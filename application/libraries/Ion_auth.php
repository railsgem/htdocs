...
class Ion_auth
{
	 ...
	 * @var array
	 **/
	public $_cache_user_in_privilege;
	...
	public function __construct()
	{
		...
		$this->_cache_user_in_privilege =& $this->ion_auth_model->_cache_user_in_privilege;
	 ...
	 * in_privilege
	 *
	 * @param mixed privilege(s) to check
	 * @param bool user id
	 * @param bool check if all privileges is present, or any of the privileges
	 *
	 * @return bool
	 * @author Juno Chen
	 **/
	public function in_privilege($check_privilege, $id=false, $check_all = false)
	{
		$this->ion_auth_model->trigger_events('in_privilege');

		$id || $id = $this->session->userdata('user_id');

		if (!is_array($check_privilege))
		{
			$check_privilege = array($check_privilege);
		}

		if (isset($this->_cache_user_in_privilege[$id]))
		{
			$privileges_array = $this->_cache_user_in_privilege[$id];
		}
		else
		{
			$users_privileges = $this->ion_auth_model->get_users_privileges($id)->result();
			$privileges_array = array();
			foreach ($users_privileges as $privilege)
			{
				$privileges_array[$privilege->id] = $privilege->name;
			}
			$this->_cache_user_in_privilege[$id] = $privileges_array;
		}
		foreach ($check_privilege as $key => $value)
		{
			$privileges = (is_string($value)) ? $privileges_array : array_keys($privileges_array);

			/**
			 * if !all (default), in_array
			 * if all, !in_array
			 */
			if (in_array($value, $privileges) xor $check_all)
			{
				/**
				 * if !all (default), true
				 * if all, false
